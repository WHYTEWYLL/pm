...
def log_activity(tenant_id: str, activity_type: str, description: str, metadata: dict = None):
    ...
    with db._conn() as conn:
        ...
        if db.use_postgres:
            cursor.execute(
                ...
                VALUES (%s, %s, %s, %s, %s)
                """,
                [activity_id, tenant_id, activity_type, description, json.dumps(metadata or {})]
            )
        else:
            cursor.execute(
                """
...
def get_active_tenants() -> list:
    ...
    with db._conn() as conn:
        if db.use_postgres:
            ...
            return [row[0] if isinstance(row, tuple) else row["id"] for row in cursor.fetchall()]
...
def get_tenant_tier(tenant_id: str) -> str:
    ...
    with db._conn() as conn:
        if db.use_postgres:
            cursor = conn.cursor()
            cursor.execute("SELECT subscription_tier FROM tenants WHERE id = %s", [tenant_id])
            row = cursor.fetchone()
            return row[0] if row else "free"
        else:
            cursor = conn.cursor()
            cursor.execute("SELECT subscription_tier FROM tenants WHERE id = ?", [tenant_id])
            row = cursor.fetchone()
            return row[0] if isinstance(row, tuple) else row.get("subscription_tier", "free") if row else "free"
...
def ingest_slack_for_tenant(self, tenant_id: str):
    ...
    try:
        ...
        if not settings.get("auto_sync", True):
            logger.info(f"Skipping Slack sync for tenant {tenant_id} - auto_sync disabled")
...
def ingest_linear_for_tenant(self, tenant_id: str):
    ...
    try:
        ...
        settings = get_workflow_settings(tenant_id)
        if not settings.get("auto_sync", True):
            logger.info(f"Skipping Linear sync for tenant {tenant_id} - auto_sync disabled")
            return {"status": "skipped", "reason": "auto_sync disabled"}
...
def ingest_github_for_tenant(self, tenant_id: str):
    ...
    try:
        ...
        settings = get_workflow_settings(tenant_id)
        if not settings.get("auto_sync", True):
            logger.info(f"Skipping GitHub sync for tenant {tenant_id} - auto_sync disabled")
            return {"status": "skipped", "reason": "auto_sync disabled"}
        ...
        logger.info(f"GitHub ingestion completed for tenant {tenant_id}: prs={len(prs)}, issues={len(issues)}")
        return {
            "status": "success",
            "prs_fetched": len(prs),
...
def daily_ingestion_for_all_tenants():
    """Run daily ingestion for all active tenants (full sync)."""
    tenant_ids = get_active_tenants()
    ...
    logger.info(f"Daily ingestion scheduled for {scheduled} tenants")
    return {"status": "scheduled", "tenants": scheduled}
    ...
    with db._conn() as conn:
        if db.use_postgres:
            ...
            if expired_tenant_ids:
                ...
                expired_count = len(expired_tenant_ids)
        else:
            cursor = conn.cursor()
            now = datetime.now(timezone.utc).isoformat()
            cursor.execute(
...
def send_standup_dm_for_user(self, tenant_id: str, user_email: str):
    ...
    try:
        ...
        logger.exception(f"Failed to send standup DM to {user_email} for tenant {tenant_id}")
...
def send_standups_for_tenant(self, tenant_id: str):
    ...
    try:
        ...
        if not settings.get("daily_standup", False):
            logger.info(f"Skipping standups for tenant {tenant_id} - daily_standup disabled")
...
celery_app.conf.beat_schedule = {
    ...
    "hourly-sync": {
        "task": "app.jobs.ingestion.hourly_sync_for_all_tenants",
    ...
    "morning-standups": {
        "task": "app.jobs.ingestion.send_morning_standups_for_all_tenants",
        "schedule": crontab(hour=9, minute=0),
    },
    # Expire trials at midnight UTC
    "expire-trials": {
